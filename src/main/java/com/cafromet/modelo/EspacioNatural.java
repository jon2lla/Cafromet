package com.cafromet.modelo;
// Generated 27 dic. 2020 22:44:50 by Hibernate Tools 5.4.21.Final

import java.util.HashSet;
import java.util.Set;

/**
 * EspacioNatural generated by hbm2java
 */
@SuppressWarnings("rawtypes")
public class EspacioNatural implements java.io.Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1574153843676489849L;
	private int idEspacio;
	private String nombre;
	private String descripcion;
	private String tipo;
	private String direccion;
	private String categoria;
	private Float latitud;
	private Float longitud;
	private Set municipio_EspacioNaturals = new HashSet(0);

	public EspacioNatural() {
	}

	public EspacioNatural(int idEspacio) {
		this.idEspacio = idEspacio;
	}

	public EspacioNatural(int idEspacio, String nombre, String descripcion, String tipo, String direccion,
			String categoria, Float latitud, Float longitud, Set municipio_EspacioNaturals) {
		this.idEspacio = idEspacio;
		this.nombre = nombre;
		this.descripcion = descripcion;
		this.tipo = tipo;
		this.direccion = direccion;
		this.categoria = categoria;
		this.latitud = latitud;
		this.longitud = longitud;
		this.municipio_EspacioNaturals = municipio_EspacioNaturals;
	}

	public int getIdEspacio() {
		return this.idEspacio;
	}

	public void setIdEspacio(int idEspacio) {
		this.idEspacio = idEspacio;
	}

	public String getNombre() {
		return this.nombre;
	}

	public void setNombre(String nombre) {
		this.nombre = nombre;
	}

	public String getDescripcion() {
		return this.descripcion;
	}

	public void setDescripcion(String descripcion) {
		this.descripcion = descripcion;
	}

	public String getTipo() {
		return this.tipo;
	}

	public void setTipo(String tipo) {
		this.tipo = tipo;
	}

	public String getDireccion() {
		return this.direccion;
	}

	public void setDireccion(String direccion) {
		this.direccion = direccion;
	}

	public String getCategoria() {
		return this.categoria;
	}

	public void setCategoria(String categoria) {
		this.categoria = categoria;
	}

	public Float getLatitud() {
		return this.latitud;
	}

	public void setLatitud(Float latitud) {
		this.latitud = latitud;
	}

	public Float getLongitud() {
		return this.longitud;
	}

	public void setLongitud(Float longitud) {
		this.longitud = longitud;
	}

	public Set getMunicipio_EspacioNaturals() {
		return this.municipio_EspacioNaturals;
	}

	public void setMunicipio_EspacioNaturals(Set municipio_EspacioNaturals) {
		this.municipio_EspacioNaturals = municipio_EspacioNaturals;
	}

}
